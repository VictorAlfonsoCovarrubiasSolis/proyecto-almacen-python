...
def regresar():#para poder regresar al menuu
...
idcambiar = cone.valor_especifico(conexion, tabla, "id", "nombre", invcambiar)#se consigue la categoria de lo seleccionado a cambiar
categoriacambiar = cone.valor_especifico(conexion, tabla, "categoria", "nombre", invcambiar)#se consigue la categoria de lo seleccionado a cambiar
cantidadcambiar = cone.valor_especifico(conexion, tabla, "cantidad", "nombre", invcambiar)#se consigue la cantidad de lo seleccionado a cambiar
...
minimocambiar = cone.valor_especifico(conexion, tabla, "minimo", "nombre", invcambiar)#se consigue el minimo de lo seleccionado a cambiar
...
def insertar_usado(id, nombre, categoria, cantidad, desecho, medida):
    ...
    consulta = "INSERT INTO usados (idproducto, nombre, categoria, cantidad, desecho, medida, fecha) VALUES (%s, %s, %s, %s, %s, %s, %s)" #para insertar los valores, especificamos las columnas.
    cursor.execute(consulta, (id, nombre, categoria, cantidad, desecho, medida, fecha))#ejecucion de la consulta
        ...
        if cantidad.isdigit():#se checa que se entero en caso de que el usuario escriba la cantidad
            if cant > 0:#la cantidad debe ser positiva
                if cant <= cantidadcambiar[0]:#la cantidad que se quiere usar debe ser menor o igual a la cantidad del producto, como la cantidad del seleccionado es una tupa se usa indice
                    messagebox.showinfo("Accion realizada", "{} {}, restada de inventario".format(tabla, invcambiar))
                    insertar_usado(idcambiar[0], invcambiar, categoriacambiar[0], cantidad, desecho, medidacambiar[0])#se manda a registrar en usados
                    cone.modificarvalor_especifico(conexion, tabla, "cantidad", "nombre", invcambiar, cantidadcambiar[0] - cant)#se cambia el valor anterior de cantidad por lo que sobra de la resta de lo que se tenia y lo que se quito
