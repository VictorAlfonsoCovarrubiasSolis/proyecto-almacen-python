...
def modificarinv(conexion, tabla, nombre, nuevonombre, nuevacategoria, nuevacantidad, nuevamedida, nuevominimo):
    cursor = conexion.cursor()#para poder hacer lñas consultas sql
    consulta = "UPDATE {} SET nombre = %s, categoria = %s, cantidad = %s, medida = %s, minimo = %s WHERE nombre = %s".format(tabla) #consulta
    cursor.execute(consulta, (nuevonombre, nuevacategoria, nuevacantidad, nuevamedida, nuevominimo, nombre))#ejecucion de la consulta 
    conexion.commit()#para que se actualice la base de datos
    cursor.close()#para liberar recursos (memoria)
