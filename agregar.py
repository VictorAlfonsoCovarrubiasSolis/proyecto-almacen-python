    ...
    if cantidad:#si tienen informacion los campos procede al registro
        if cantidad.isdigit():#se checa que se entero en caso de que el usuario escriba la cantidad
            if cant > 0:#se checa que sea una cantidad positiva
                messagebox.showinfo("Accion realizada", "{} {}, agregado al inventario".format(tabla, invcambiar))
                cone.modificarvalor_especifico(conexion, tabla, "cantidad", "nombre", invcambiar, cantidadcambiar[0] + cant)#se cambia el valor anterior de cantidad por el ingresado pro el usuario
